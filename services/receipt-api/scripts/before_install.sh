...
if [-d "$DEPLOY_DIR"]; then
  ...
  echo "Limpando diretorio de deplou '$DEPLOY_DIR' (excluindo $PRESERVE_DIR_RELATIVE/)..."

  find . -mindepth 1 ! -ipath "./$PRESERVE_DIR_RELATIVE*" -delete || true
  echo "Limpeza concluida."

else
  echo "Diiretorio de deploy '$DEPLOY_DIR' nao existe (primeiro deploy?). Pulando limpeza."

  mkdir -p "$DEPLOY_DIR"
fi

echo "--- Hook BeeforeInstall finalizaado ---"
exit 0
