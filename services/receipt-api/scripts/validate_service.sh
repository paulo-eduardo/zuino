...
while [$RETRY_COUNT -lt $MAX_RETRIES]; do
  if curl -f -s -o /dev/null "$HEALTH_CHECK_URL"; then
    echo "Validacao bem-sucedida na tentativa $((RETRY_COUNT + 1))! O servico esta respondendo na port $APP_PORT."
    exit 0
  else
    RETRY_COUNT=$((RETRY_COUNT + 1))
    if [ $RETRY_COUNT - lt $MAX_RETRIES ]; then
      echo "Tentativa $RETRY_COUNT falhou. Tentando novamente em $RETRY_INTERVAL segunds..."
